...
test_glm_tweedies <- function() {
  if (requireNamespace("tweedie")) {
    ...
    pow <- c(1.1, 2, 3, 4) # variance power range
    phi <- c(1, 1, 2, 3)   # dispersion factor range
...
compareH2ORGLM<-function(vpower, lpower, x, y, hdf, df, truedisp, tolerance=5e-6) {
  ...
  print(paste("H2O model dispersion estimate", h2omodel@model$dispersion, sep=":"))
  print(paste("R model dispersion estimate", summary(rmodel)$dispersion, sep=":"))
  h2oDiff = abs(h2omodel@model$dispersion-truedisp)
  rDiff = abs(summary(rmodel)$dispersion-truedisp)
  expect_true(h2oDiff <= rDiff, info=paste("H2O dispersion estimation error", h2oDiff, 
                                           "R dispersion estimation error", rDiff, sep=":"))
}
